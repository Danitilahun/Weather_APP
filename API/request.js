const countries = {
  ...
  "French Southern and Antarctic Lands": [-49.280366, 69.348557],
  ...
  "Northern Cyprus": [35.126413, 33.429859],
...
async function fetchAndStoreWeatherData(latitude, longitude) {
  try {
    ...
    const weatherData = mapResponseToSchema(response.data);

    await WeatherData.create(weatherData);
    console.log("Weather data stored successfully.");
  } catch (error) {
    console.error("Failed to fetch and store weather data:", error.message);
  }
}

async function fetchWeatherDataForCountries() {
  for (const country in countries) {
    const [latitude, longitude] = countries[country];

    try {
      console.log(`Fetching weather data for ${country}...`);
      await fetchAndStoreWeatherData(latitude, longitude);
    } catch (error) {
      console.error(
        `Failed to fetch weather data for ${country}:`,
        error.message
      );
    }
  }
...
// Define the countries and their coordinates
// Start fetching weather data for all countries
// fetchWeatherDataForCountries(countries);

module.exports = fetchWeatherDataForCountries;
